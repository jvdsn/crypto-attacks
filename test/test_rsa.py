...
class TestRSA(TestCase):
    ...
    def test_wiener_attack_lattice(self):
        ...
        N = p * q
        phi = (p - 1) * (q - 1)
        d = [82761005795391335538553585681906170948593770602013160773436568731880824979889, 73347387000388290096937818299618529800127010868555446700505573861582571064707, 89095171139400657821823949655743896117454141824105050561325562634556941719643]
        e = [pow(dk, -1, phi) for dk in d]
        p_, q_ = wiener_attack_lattice.attack_multiple_exponents(N, e, d_bit_length=512)
        self.assertIsInstance(p_, int)
        self.assertIsInstance(q_, int)
        self.assertEqual(N, p_ * q_)
